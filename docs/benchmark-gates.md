...
4. `mc_cpu_european_call_rust`, `mc_cpu_european_call_parameter_sweep_rust`, `inventory_cpu_periodic_review_rust`, `mc_cpu_down_and_out_call_rust`, `mc_cpu_lookback_call_rust`, `mc_cpu_american_put_lsm_rust`, `mc_cpu_bermudan_put_lsm_rust`, `mc_cpu_heston_european_call_rust`, and `mc_cpu_merton_jump_diffusion_call_rust` must be present in benchmark results.
...
15. `inventory_cpu_periodic_review_rust` must report positive runtime and path-period throughput plus a finite positive `mean_total_cost`. This is a presence and health gate, not a competitiveness claim.
16. When `inventory_cpu_periodic_review_numpy` or `inventory_cpu_periodic_review_numba` rows are available, the Rust inventory CPU runtime must be faster on the tracked `10,000 paths x 104 periods` workload.
