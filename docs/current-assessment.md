...
- Phase 4 AI-agent-native surface:
  ...
  - planner-intelligence tools for measured evidence, cost frontiers, method comparisons, why-not-faster explanations, and MLMC calibration
...
- Phase 7.1/7.2 now have CPU Longstaff-Schwartz American-put and
  custom-schedule Bermudan-put reference surfaces, lower-bound European-put
  fixtures, benchmark rows, and method capability entries
...
- broad market-leader coverage across more simulation families such as
  high-precision early-exercise references, advanced adjoint Greeks, adaptive
  MLMC, or scientific UQ workflows
...
- Rust CPU European step-wise: `15.242 ms`
...
- Metal European step-wise: `1.483 ms`
- Metal arithmetic Asian step-wise: `2.743 ms`
- Metal down-and-out step-wise: `1.481 ms`
...
### 5. Planner intelligence is now artifact-backed

`planner_choice_accuracy_measured` is now `100%` on the current local scenario
set. The planner evidence layer exposes measured winner records, cost
frontiers, method comparisons, why-not-faster explanations, and
MLMC/MLQMC calibration records through Python and agent-safe wrappers. This is
strong for the tracked suite, but it is still hardware-local and must be
refreshed on target machines before production deployment claims.
...
- keep benchmark coverage that records estimated vs realized MLMC/MLQMC error
...
### Priority 4: Keep broadening planner evidence

Needed next:

- add more measured winner scenarios as new workload families land
- separate support from recommendation more clearly
- ingest hardware-specific observations once CUDA comes online
...
2. Use measured structured-sampling winners to keep improving method recommendations.
3. Broaden adaptive MLMC/MLQMC tolerance calibration beyond the first arithmetic Asian surface.
...
- over-generalize `100%` tracked planner accuracy beyond the measured local suite
