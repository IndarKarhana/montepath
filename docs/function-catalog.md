...
| `NativeArtifactMetadata` | `crates/mc-core/src/backend/mod.rs` | Describe staged native kernel artifacts for GPU backends. | Output fields include kernel family, entry point, source module, compile request status, compile success status, and optional compiled module path. | Deterministic for same build and environment. | Compile success still depends on toolchain availability; native execution breadth still varies by backend. | Strong future agent/tool diagnostic surface. |
...
| `AppleMetalBackend::execute` | `crates/mc-core/src/backend/metal.rs` | Execute the staged v1 Metal kernel path on macOS when `metal-native` is enabled, using cached in-process Metal pipelines when native support is available and falling back otherwise. | Inputs: `CompiledArtifact`, `BackendExecutionInput::{EuropeanCall, ArithmeticAsianCall}`. Output: `RunOutput`. | Deterministic for identical config and seed. | Native path is still narrow to the staged GBM option family, but now covers both European and arithmetic Asian step-wise workloads. | Strong future tool/runtime surface, with a real native Apple GPU execution path. |
