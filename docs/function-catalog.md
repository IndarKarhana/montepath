...
| `AppleMetalBackend::execute` | `crates/mc-core/src/backend/metal.rs` | Execute the staged v1 Metal kernel path on macOS when `metal-native` is enabled, using cached in-process Metal pipelines when native support is available and falling back otherwise. | Inputs: `CompiledArtifact`, `BackendExecutionInput::EuropeanCall`. Output: `RunOutput`. | Deterministic for identical config and seed. | Native path is currently narrow to the first standard step-wise European-call workload and still falls back when runtime support is unavailable. | Strong future tool/runtime surface, with a now-real native Apple GPU execution path. |
